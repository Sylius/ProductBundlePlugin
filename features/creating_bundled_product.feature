...
Feature: Creating a product in store which is a bundle of other products
    ...
    @ui @javascript
    Scenario: Creating an empty product bundle
      When I want to create a new bundled product
      And I specify its code as "WHISKEY_EMPTY_PACK"
      And I name it "Whiskey empty pack" in "English (United States)"
      And I set its slug to "whiskey-empty-pack" in "English (United States)"
      And I set its price to "$10.00" for "United States" channel
      And I set its original price to "$20.00" for "United States" channel
      And I add it
